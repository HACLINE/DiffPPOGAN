...
class DiffPPOGANAdvScheduleR3Workspace(BaseWorkspace):
    def __init__(self):
        pass
    ...
    def setup(self, cfg):
        ...
        self.curve_every = cfg.curve_every
        self.reset_discriminator = cfg.reset_discriminator
        self.grad_penalty_gamma = cfg.grad_penalty_gamma
    ...
    def train_model(self):
        ...
        # adversarial_loss = torch.nn.Softplus()
        ...
        minimum_FID = {"model": None, "critic": None, "discriminator": None, 'model_update_epoch': None, 'FID': np.inf}
        FID_list = []
        for epoch in epoch_bar:
            ...
            if self.curve_every > 0 and epoch % self.curve_every == 0:
                ...
                metrics['Curve/Adv_Schedule'] = wandb.Image(plt)
                plt.close()
            ...
            for start in tqdm(range(0, b_size - self.batch_size + 1, self.batch_size), desc='Discriminator Update', leave=False):
                ...
                d_loss_weights = extract_time_index(self.discriminator_schedule, batch_timesteps, torch.Size([self.batch_size, 1]))
                original_loss = adversarial_loss(fake_validity, fake) * d_loss_weights + adversarial_loss(real_validity, valid) * d_loss_weights
                original_loss = original_loss.mean(dim=(0, 1))
                ...
                original_loss_total += original_loss.item()
                r1_total += r1.item()
                r2_total += r2.item()
                cnt += 1
            ...
            original_loss = adversarial_loss(fake_validity, torch.zeros_like(fake_validity)) + adversarial_loss(real_validity, torch.ones_like(real_validity))
            original_loss = original_loss.mean(dim=(0, 1))
            r1 = self.zero_centered_grad_penalty(batch_gt_obs, real_validity).mean()
            r2 = self.zero_centered_grad_penalty(next_obs, fake_validity).mean()
            ...
            metrics.update({
                ...
                'loss/original_loss': original_loss_total / cnt,
                'loss/r1': r1_total / cnt,
                'loss/r2': r2_total / cnt,
            ...
            if epoch % self.snapshot == 0:
                ...
                torch.save(self.critic.state_dict(), self.models_dir / f"critic_{'Lat_' if self.vae is not None else ''}{self.dataset}_epoch{epoch}.pt")
                if minimum_FID['model'] is not None:
                    torch.save(minimum_FID['model'], self.models_dir / f"model_{'Lat_' if self.vae is not None else ''}{self.dataset}_minimum_FID.pt")
                    torch.save(minimum_FID['discriminator'], self.models_dir / f"discriminator_{'Lat_' if self.vae is not None else ''}{self.dataset}_minimum_FID.pt")
                    torch.save(minimum_FID['critic'], self.models_dir / f"critic_{'Lat_' if self.vae is not None else ''}{self.dataset}_minimum_FID.pt")
            ...
            if self.eval_fid_every > 0 and self.model_update_epoch % self.eval_fid_every == 0 and self.model_update_epoch not in FID_list:
                ...
                if fid_score < minimum_FID['FID']:
                    minimum_FID['model'] = copy.deepcopy(self.model.state_dict())
                    minimum_FID['critic'] = copy.deepcopy(self.critic.state_dict())
                    minimum_FID['discriminator'] = copy.deepcopy(self.discriminator.state_dict())
                    minimum_FID['model_update_epoch'] = self.model_update_epoch
                    minimum_FID['FID'] = fid_score
                metrics['FID/minimum_FID'] = minimum_FID['FID']
                metrics['freeze/reset'] = 0
                if fid_score > minimum_FID['FID'] + self.reset_gap:
                    self.model.load_state_dict(minimum_FID['model'])
                    self.critic.load_state_dict(minimum_FID['critic'])
                    if self.reset_discriminator:
                        self.discriminator.load_state_dict(minimum_FID['discriminator'])
                    cool_down = self.cool_down
